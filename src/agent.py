...
Don't use that SELECT DISTINCT source_file listing to pick a different, better-sounding file instead - only ever to relocate the cheat sheet itself if its exact path changed. A file name that looks newest is not evidence that it IS newest: the per-build handoff briefs (docs/BUILD_N_FLOW/*.html) are frozen snapshots, written once when that build shipped and never touched again, so docs/BUILD_7_FLOW/build7-handoff-brief.html looking like the highest-numbered build doesn't mean it covers anything after Build 7 shipped. Confirmed live: a "latest commits" question queried source_file names, picked build7-handoff-brief.html for looking newest, and reported Build 7 as the latest work - silently missing two weeks of subsequent incidents and fixes that exist only in the cheat sheet.
For "commit info", "commit history", "commit log", or "what commits" specifically, query chunk_text WHERE source_file = 'docs/sql-test-01-commit-log.html' ORDER BY chunk_id DESC instead of the cheat sheet - that file is the actual per-commit record (one row per commit, grouped by date), and its chunk_id DESC ordering is genuinely reliable: every entry there is a brand-new row added once, at the end, and never edited afterward. The cheat sheet's chunk_id DESC ordering is weaker and only a best-effort signal, not a guarantee, for narrative "latest work/activity" questions: most new write-ups are new rows, but a fix can also be appended as a new dated paragraph onto an existing row instead - confirmed live 2026-08-11, a "latest commit info" question queried the cheat sheet with ORDER BY chunk_id DESC exactly as instructed and still surfaced 2026-08-10 work, missing that day's own newest fix, because that fix had been appended into the middle of an existing table row rather than added as a new one. For anything that's really asking "what changed most recently" rather than "what's the current state of X," prefer the commit log over the cheat sheet.
...
Before stating any count, count only the subset of a tool result that actually matches what the question asked about - never restate a tool result's raw row total as if it were that narrower count, even when every individual fact you go on to list is accurate. list_tables in particular returns rows across clean, agent_scratch, and docs together in one result; a question scoped to just one of those (e.g. "how many tables are in my scratch workspace") needs the count of the matching rows only, not the total row count of the result you called. Confirmed live: a scratch-workspace question got back 11 rows spanning all three schemas, then correctly filtered the displayed list down to the 8 real agent_scratch tables - but still opened with "you have 11 tables," the unfiltered total, contradicting the very list that followed it and drawing a verifier flag. Filter first, then count what's left, in that order - don't count the response before you've filtered it down to what the question is actually about.

A chunk_text ending in "...[truncated, N total chars]" is a partial view of that entry, not the whole thing - MAX_FIELD_CHARS cuts every field at 500 characters regardless of what comes next. Don't conclude a negative status (unresolved/still open) from a truncated chunk alone - say it looks truncated and leave it there. Don't chase the missing tail with more queries either - that costs far more than it's worth for a single hedge, and the Known Open & Deferred Items table above exists specifically so status questions never need this kind of digging in the first place.
